...
const Dashboard = () => {
  ...
  const adminCards = [['Institution publications', stats?.publications_count, 'bi-journal-text', '/publications'], ['Researchers', stats?.researchers_count, 'bi-people', '/researchers'], ['Citations', stats?.citations_count, 'bi-quote', '/citations'], ['Pending requests', stats?.pending_requests_count, 'bi-hourglass-split', '/collaborations'], ['Collaborations', stats?.collaboration_count, 'bi-diagram-3', '/collaborations'], ['Active projects', stats?.active_projects || 0, 'bi-folder2-open', '/collaborations']];
  const cards = role === 'institution_admin' ? adminCards : researcherCards;
  ...
  return <div className="container py-5">
    <div className="hero-section"><h1><i className="bi bi-graph"></i> Welcome, {user?.full_name}!</h1><p className="lead">{role === 'institution_admin' && stats?.institution_name ? `${stats.institution_name} · ` : ''}{title}</p></div>
    ...
    {['researcher', 'system_admin'].includes(role) && <AIRecommendations />}
